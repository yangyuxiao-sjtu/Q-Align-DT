...
from decision_transformer.models.decision_convformer import Decision_convformer
...
def experiment(
    ...
    if env_name == 'hopper':
        ...
        #     clip_range=[-3,4]
        # if 'expert' in dataset:
        #     env_targets=[4500,3600,1800] # This is because model can achieve higher than 3600 in expert dataset
        ...
        max_ep_len = 1000
        if variant['rtg_scale'] is not None:
    ...
    if variant['clip_min'] is not None or variant['clip_max'] is not None:
        default_min, default_max = (clip_range if clip_range is not None else (None, None))
        clip_min = default_min if variant['clip_min'] is None else variant['clip_min']
        clip_max = default_max if variant['clip_max'] is None else variant['clip_max']
        if clip_min is None or clip_max is None:
            raise ValueError(
                f"clip_range override requires both bounds to be defined; got clip_min={clip_min}, clip_max={clip_max}"
            )
    ...
    if not os.path.exists(os.path.join(variant['save_path'], exp_prefix)):
        pathlib.Path(os.path.join(variant['save_path'], exp_prefix)).mkdir(
            parents=True,
            exist_ok=True,
        )
    ...
    dataset_path = os.path.join(
        variant['data_root'],
        f'{env_name}-{dataset}-v{dversion}.pkl',
    )
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        for i in range(batch_size):
            ...
            # if 'hopper-medium' in gym_name and 'replay' not in gym_name:
    ...
    elif variant['exp_name']=='dc':
        model = Decision_convformer(
            state_dim=state_dim,
            act_dim=act_dim,
            max_length=K,
            max_ep_len=max_ep_len,
    ...
    critic = Critic(
        ...
        state_mean=state_mean,
        state_std=state_std,
        state_adapter=variant['critic_state_adapter'],
    )
    ...
    trainer = Trainer(
        ...
        noise_q=variant['noise_q'],
        critic_update=variant.get("critic_update",1),
        use_target_q_stop=not variant['no_target_q_stop'],
        target_q_stop_start_epoch=variant['target_q_stop_start_epoch'],
        target_q_stop_window=variant['target_q_stop_window'],
        target_q_stop_relative_drop=variant['target_q_stop_relative_drop'],
        target_q_stop_patience=variant['target_q_stop_patience'],
    )
#(sum(env_targets)/len(env_targets))/scale,

    best_ret = -10000
    best_nor_ret = -1000
    best_iter = -1
    log_to_wandb = not variant.get('disable_wandb', False)
    ...
    if variant.get('wandb_name_prefix'):
        wandb_name = f"{variant['wandb_name_prefix']}-{wandb_name}"
    wandb_name_suffix = variant.get('wandb_name_suffix')
    if wandb_name_suffix == 'auto':
        wandb_name_suffix = timestr
    if wandb_name_suffix:
        wandb_name = f"{wandb_name}-{wandb_name_suffix}"
    project_name = variant.get('wandb_project') or f"{variant['exp_name']} Gym"
    ...
    if log_to_wandb:
        ...
        if variant.get('wandb_group'):
            wandb_init_kwargs['group'] = variant['wandb_group']
    ...
    for iter in range(variant['max_iters']):
        ...
        if ret >= best_ret:
            ...
            best_iter = iter + 1
        outputs['run/current_best_return_mean'] = best_ret
        outputs['run/current_best_normalized_score'] = best_nor_ret
        outputs['run/current_best_iteration'] = best_iter
        if log_to_wandb:
            wandb.log(outputs)
        logger.log(f'Current best return mean is {best_ret}, normalized score is {best_nor_ret}, Iteration {best_iter}')
        if variant['early_stop'] and iter >= variant['early_epoch']:
            break
    logger.log(f'The final best return mean is {best_ret}')
    logger.log(f'The final best normalized return is {best_nor_ret}')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--critic_state_adapter",
        type=str,
        default="identity",
        choices=["identity", "unnormalize"],
    )
    ...
    parser.add_argument("--no_target_q_stop", action="store_true", default=False)
    parser.add_argument("--target_q_stop_start_epoch", type=int, default=300)
    parser.add_argument("--target_q_stop_window", type=int, default=3)
    parser.add_argument("--target_q_stop_relative_drop", type=float, default=0.005)
    parser.add_argument("--target_q_stop_patience", type=int, default=2)
    parser.add_argument("--rtg_scale",type = float, default =None)
    ...
    parser.add_argument("--noise_q", type = float, default =None)
    ...
    parser.add_argument("--wandb_name_suffix", type=str, default=None)
